/* =========================================================
   个人主页 · 交互脚本
   ---------------------------------------------------------
   音乐播放器：默认 4 首，改歌只需修改下面的 SONGS 数组。
...
// ===== 1. 歌曲配置（固定 4 首，改这里就行）=====
// file 只写"文件名"（要带扩展名），不用带 music/ 前缀 —— 只要文件确实在 music/ 里。
// 纯数字（1.m4a）、中文、空格、全角括号都会自动转义，不用自己处理。
// 格式随意：mp3 / m4a / ogg 这些浏览器能放的都行，混着放也没问题。
// name 是界面上显示的歌名 —— 这里跟 music/ 里的文件名保持一致。
// 换歌：把新文件放进 ./music/，然后改下面 4 行的 file 即可。
const SONGS = [
  { name: "1", artist: "SOOL", file: "1.m4a" },
  { name: "2", artist: "SOOL", file: "2.mp3" },
  { name: "3", artist: "SOOL", file: "3.mp3" },
  { name: "4", artist: "SOOL", file: "4.mp3" },
];
